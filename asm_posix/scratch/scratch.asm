...
print_int:
	; void print_int(int {rdi}, int {rax})
	; Print integer in {rax} to file descriptor {rdi}.

	; void print_int(int {rdi}, int {rsi})
	; Print integer in {rsi} to file descriptor {rdi}.
	; #
	; By converting each digit to ASCII char and writing to standard output
	; See also "https://github.com/lloydlobo/SCHIZONE/blob/main/ex/ex005_printing_integers/ex005a_printing_ints/code.asm"

	;Save registers
	;push rdi; File descriptor (stdin, stdout, stderr)
	;{---todo-----
	;push rsi; Integer to print
	push  rax; Integer to print
	;----todo----}

	mov rcx, buffer
	add rcx, (N_BUFFER64 - 1); Start from the end of the buffer
	mov byte [rcx], 0; Null-terminate the string
	mov rbx, BASE_10; > To convert the number to its decimal (base 10) string
	;   >             > representation and use the remainders to build the string.

	; TODO: Do all arithmentic operations in {rax}
...
.loop_print_newline:
	...
	;mov rdi, rdi; File descriptor
	mov  rsi, NEWLINE_DB
	mov  rdx, 1; Number of bytes to write. Writing only one byte (the newline character)
	syscall
...
_start:
	;    Example: Print 12345
	mov  rdi, SYS_FD_STDOUT
	mov  rax, 12345; mov  rsi, 12345; todo
	call print_int; void print_int(int {rdi}, int {rsi})

	;    Example: Print 18446744073709551615
	mov  rdi, SYS_FD_STDOUT
	mov  rax, MAX_INT;; mov  rsi, MAX_INT; todo
	call print_int; void print_int(int {rdi}, int {rsi})
